...
(again, even if we control for factors such as passenger class). A variable such as this can introduce an element of circularity into the training process, with the result of a badly overfit model. One of my aims was to see if I could make such an average (or something like this
average, based on surnames) a useful predictor, and to see if it is possible to overcome the model-overfitting issues (associated with the circularity issue) that such a variable can
introduce.
